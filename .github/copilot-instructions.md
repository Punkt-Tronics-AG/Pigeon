...
- **Never rebase, amend, squash, force-push or otherwise rewrite history
  without explicit approval from the user.** This includes
  `git commit --amend`, `git rebase`, `git reset`, `git push --force*`.
- **Never create commits or push without being asked to.** Leave changes in
  the working tree and let the user decide.
- When resources are removed upstream (drawables, dimens, strings …), **do not
  restore the old resources** in Pigeon variants – adapt the `-small` layout
  to match how the current original Signal layout handles it.
